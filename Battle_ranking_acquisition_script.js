(async function () {
    ...
    const BaseURL = "https://ongeki-net.com/ongeki-mobile/ranking/battlePoint/?rankingType=";
    ...
    try {
        ...
        // 自分のランキング情報のブロックを取得
        const userBlock = doc.querySelector('.ranking_user_block .gray_line.f_b');
        let myRankImg = ''; // 自分のランク画像（HTML）用変数
        let myScore = ''; // 自分のスコア文字列用変数
        if (userBlock) {
            const img = userBlock.querySelector('img');
            if (img) {
                // ランク画像をHTMLとして保存
                myRankImg = `<img src="${img.src}" class="h_35 v_m">`;

                // テキストから数字とカンマ以外を除去してスコアとして保存
                myScore = userBlock.textContent.replace(/[^\d,]/g, '') || '不明';
            }
        }
        ...
        // 各行を処理してランキングデータを抽出
        rows.forEach((row) => {
            if (row.classList.contains("ranking_title_block")) return; // タイトルブロックはスキップ

            const cells = row.querySelectorAll('td');
            if (cells.length < 5) return; // セル数が少ない行は無視
            ...
            if (rankHtml && name && score) {
                rankingData.push({ rankHtml, name, characterImg, score });
            }
        });
        ...
        const htmlContent = `
            ...
            <body>
                <h1>オンゲキ バトルポイントランキング - ${typeNames[R_Type.toString()]}</h1>

                <div class="my-info">
                    <div class="my-info-title">あなたのバトルランク／バトルポイント</div>
                    <div>${myRankImg} ${myScore}</div>
                </div>
                    ...
                    <tbody>
                        ${rankingData.map(entry => `
                            <tr>
                                ...
                                <td>${entry.name}</td>
                                <td><img src="${entry.characterImg}" class="h_35"></td>
                                <td>${entry.score}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                ...
                <script>
                    function downloadHTML() {
                        ...
                        a.href = url;
                        a.download = 'ongeki_battlepoint_ranking_${NowDate}_${typeNames[R_Type.toString()]}.html';
                        a.click();
